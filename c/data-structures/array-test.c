...
void testDArray(int expectSize, const char* expectStr, int precision, const char* s) {
  int size;
  double *array = darrayNewByStr(s, &size);

  EXPECT_EQ_INT(expectSize, size);
  EXPECT_EQ_STRING_AND_FREE_ACTUAL(expectStr, darrayToString(array, size, precision));

  free(array);
}

void testDArrayRoundTrip(int expectSize, const char* s, int precision) {
  testDArray(expectSize, s, precision, s);
}
...
int main(void) {
  ...
  testDArrayRoundTrip(3, "[1.01,2.02,3.03]", 2);
